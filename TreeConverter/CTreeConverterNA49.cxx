#ifndef CTREECONVERTERNA49_CXX
#define CTREECONVERTERNA49_CXX
...
#include <TStyle.h> // QA
#include "CTreeConverterNA49.h"

using namespace std;

CTreeConverterNA49::CTreeConverterNA49 () {
	initFlag_ = 0;
}


CTreeConverterNA49::CTreeConverterNA49 (TString inputFileName, TString outputFileName, TString inputTreeName) {
	initFlag_ = 0;
...
void CTreeConverterNA49::SetInputFileName (TString inputFileName) {
    inputFileName_ = inputFileName;
}

void CTreeConverterNA49::SetOutputFileName (TString outputFileName) {
    outputFileName_ = outputFileName;
}

void CTreeConverterNA49::SetInputTreeName (TString inputTreeName) {
    inputTreeName_ = inputTreeName;
}

Bool_t CTreeConverterNA49::SetInputFile () {
...
Bool_t CTreeConverterNA49::SetOutputFile () {
...
Bool_t CTreeConverterNA49::SetInputTree () {
...
void CTreeConverterNA49::SetMhRange (Int_t mhMin, Int_t mhMax) {
    mhMin_ = mhMin;
    mhMax_ = mhMax;
}

void CTreeConverterNA49::SetCentRange (Float_t centMin, Float_t centMax) {
    centMin_ = centMin;
    centMax_ = centMax;
}

void CTreeConverterNA49::SetNbinsMh (Int_t nBinsMh) {
	nBinsMh_ = nBinsMh;
}

void CTreeConverterNA49::SetNbinsCent (Int_t nBinsCent) {
	nBinsCent_ = nBinsCent;
}

void CTreeConverterNA49::SetdEdxSource (Int_t TPCid) {
    dEdxSource_ = TPCid;
}

void CTreeConverterNA49::SetCentralityMethod (Int_t centMethod) {
    centMethod_ = centMethod;
}

Bool_t CTreeConverterNA49::Init (){
...
Int_t CTreeConverterNA49::GetNgoodTracks () {
    Int_t nGoodTracks = 0;
    for (Int_t itrack = 0; itrack < fNPrimaryParticles; itrack++) {
        if (CheckTrackCuts (itrack)) nGoodTracks++;
    }
    return nGoodTracks;
}


Bool_t CTreeConverterNA49::CheckEventCuts () {
//    return 1; // patch
    ...
    if (GetNgoodTracks () <= 10) return 0;
    return 1;
}

Float_t CTreeConverterNA49::GetCentralityClass (Int_t mh) {
...
Float_t CTreeConverterNA49::GetCentralityClass (Float_t Eveto) {
...
void CTreeConverterNA49::SetSNN (Float_t sNN) {
    sNN_ = sNN;
}

Float_t CTreeConverterNA49::GetRapidity (Float_t pt, Float_t eta, Int_t pid) {
...
Bool_t CTreeConverterNA49::CheckTrackCuts (Int_t itrack) {
////    return 1; // patch
...
////		fitFraction = (Float_t) fPrimaryParticles_fNPoint[itrack][3] / fPrimaryParticles_fNMaxPoint[itrack][3];
...
//    if (nVTPC1 + nVTPC2 < 20 || nMTPC < 30) return 0;
////    if (nVTPC1 < 20 || nVTPC2 < 20 || nMTPC < 30) return 0;
//    if (TMath::Abs(Bx) > 3.0 || TMath::Abs(By) > 0.5) return 0;
...
Int_t CTreeConverterNA49::GetTrackPid (Int_t itrack) {
...
Bool_t CTreeConverterNA49::ConvertTree () {
	if (!Init ()) return 0;
...
//    Float_t ptMin = 0.0, ptMax = 5.0, etaMin = 0.0, etaMax = 14.0, rapMin = -2.5, rapMax = 5.0; // raw
    Float_t ptMin = 0.0, ptMax = 2.5, etaMin = 1.4, etaMax = 5.0, rapMin = -2.5, rapMax = 3.0; // cut

	TFile *histFile = new TFile (outputFileName_ + "_QA.root", "recreate");

	TH1F *hVertexX = new TH1F ("hVertexX", "Vertex X distribution;Vertex X;nEvents", 100, -0.5, 0.5);
    TH1F *hVertexY = new TH1F ("hVertexY", "Vertex Y distribution;Vertex X;nEvents", 100, -0.5, 0.5);
    TH1F *hVertexZ = new TH1F ("hVertexY", "Vertex Y distribution;Vertex X;nEvents", 100, -582.0, -580.0);
    TH1F *hVertex_chi2 = new TH1F ("hVertex_chi2", "Vertex #chi^{2};#chi^{2};nEvents", 100, 0.5, 1.05);
    TH1F *hNPoint = new TH1F ("hNPoint", "fNPoint distribution; fNPoint; nTracks", 250, 0, 250);
    TH1F *hNPoint_VTPC1 = new TH1F ("hNPoint_VTPC1", "fNPoint in VTPC1;fNPoint;nTracks", 70, 0, 70);
    TH1F *hNPoint_VTPC2 = new TH1F ("hNPoint_VTPC2", "fNPoint in VTPC2;fNPoint;nTracks", 70, 0, 70);
    TH1F *hNPoint_MTPC = new TH1F ("hNPoint_MTPC", "fNPoint in MTPC;fNPoint;nTracks", 100, 0, 100);
    TH1F *hNFitPoint = new TH1F ("hNFitPoint", "fNFitPoint distribution; fNFitPoint; nTracks", 250, 0, 250);
    TH1F *hNFitPoint_VTPC1 = new TH1F ("hNFitPoint_VTPC1", "fNFitPoint in VTPC1;fNFitPoint;nTracks", 70, 0, 70);
    TH1F *hNFitPoint_VTPC2 = new TH1F ("hNFitPoint_VTPC2", "fNFitPoint in VTPC2;fNFitPoint;nTracks", 70, 0, 70);
    TH1F *hNFitPoint_MTPC = new TH1F ("hNFitPoint_MTPC", "fNFitPoint in MTPC;fNFitPoint;nTracks", 100, 0, 100);
    TH1F *hNMaxPoint = new TH1F ("hNMaxPoint", "fNMaxPoint distribution; fNPMaxoint; nTracks", 250, 0, 250);
    TH1F *hNMaxPoint_VTPC1 = new TH1F ("hNMaxPoint_VTPC1", "fNMaxPoint in VTPC1;fNMaxPoint;nTracks", 70, 0, 70);
    TH1F *hNMaxPoint_VTPC2 = new TH1F ("hNMaxPoint_VTPC2", "fNMaxPoint in VTPC2;fNMaxPoint;nTracks", 70, 0, 70);
    TH1F *hNMaxPoint_MTPC = new TH1F ("hNMaxPoint_MTPC", "fNMaxPoint in MTPC;fNMaxPoint;nTracks", 100, 0, 100);
    TH2F *h2NPoint_NFitPoint = new TH2F ("h2NPoint_NFitPoint", "fNPoint vs fNFitPoint; fNPoint; fNFitPoint; nTracks", 250, 0, 250, 250, 0, 250);
	TH2F *h2NPoint_NMaxPoint = new TH2F ("h2NPoint_NMaxPoint", "fNPoint vs fNMaxPoint; fNPoint; fNMaxPoint; nTracks", 250, 0, 250, 250, 0, 250);
    TH1F *hFitToMaxRatio = new TH1F ("hFitToMaxRatio", "#frac{fNFitPoint}{fNMaxPoint} distribution; FitToMaxRatio; nTracks", 100, 0.55, 1.0);
    TH1F *hTrack_chi2 = new TH1F ("hTrack_chi2", "Track #chi^{2};#chi^{2};nTracks", 100, 0.0, 10.0);
    TH1F *hBx = new TH1F ("hBx", "DCA_{x} distribution;DCA_{x};nTracks", 100, -3.0, 3.0);
    TH1F *hBy = new TH1F ("hBy", "DCA_{y} distribution;DCA_{y};nTracks", 100, -0.5, 0.5);
	...
	TH2F *h2logPqdEdx = new TH2F ("h2logPqdEdx", "Energy loss vs q*log(20P); log (P); dE/dx [MIP]", 500, -10.0, 10.0, 500, 0.0, 4.0);
	...
	TH2F *h2EtaPhiPiMin = new TH2F ("h2EtaPhiPiMin", "#eta vs #phi distribution fPrimaryParticles_fTmeanCharge [] [2]for #pi-;#eta; #phi; nTracks", 500, etaMin, etaMax, 500, -PI, PI);
...
//                                                                      // 3 4
	Double_t phiVeto [4] = {0.25 * PI, 1.75 * PI, 0.75 * PI, 1.25 * PI}; // 1 4
                                                                        // 2 3
//	Float_t phiVeto [4] = {1.75 * PI, 1.25 * PI, 0.75 * PI, 0.25 * PI}; // 1 3
	...
	for (Long64_t jentry = 0; jentry < nentries; jentry++) {
		...
		hVertexX -> Fill (fVertexX);
		hVertexY -> Fill (fVertexY);
		hVertexZ -> Fill (fVertexZ);
		hVertex_chi2 -> Fill (fVertex_fPchi2);
		...
		for (Int_t itrack = 0; itrack < mh; itrack++) {
		    ...
		    h2logPqdEdx -> Fill (TMath::Log(20.0 * p) * charge, dEdx); // QA fill
            ...
            hNPoint -> Fill (fPrimaryParticles_fNPoint [itrack][3]);
            hNPoint_VTPC1 -> Fill (fPrimaryParticles_fNPoint [itrack][0]);
            hNPoint_VTPC2 -> Fill (fPrimaryParticles_fNPoint [itrack][1]);
            hNPoint_MTPC -> Fill (fPrimaryParticles_fNPoint [itrack][2]);
            hNFitPoint -> Fill (fPrimaryParticles_fNFitPoint [itrack][3]);
            hNFitPoint_VTPC1 -> Fill (fPrimaryParticles_fNFitPoint [itrack][0]);
            hNFitPoint_VTPC2 -> Fill (fPrimaryParticles_fNFitPoint [itrack][1]);
            hNFitPoint_MTPC -> Fill (fPrimaryParticles_fNFitPoint [itrack][2]);
            hNMaxPoint -> Fill (fPrimaryParticles_fNMaxPoint [itrack][3]);
            hNMaxPoint_VTPC1 -> Fill (fPrimaryParticles_fNMaxPoint [itrack][0]);
            hNMaxPoint_VTPC2 -> Fill (fPrimaryParticles_fNMaxPoint [itrack][1]);
            hNMaxPoint_MTPC -> Fill (fPrimaryParticles_fNMaxPoint [itrack][2]);
            h2NPoint_NFitPoint -> Fill (fPrimaryParticles_fNPoint [itrack][3], fPrimaryParticles_fNFitPoint [itrack][3]);
            h2NPoint_NMaxPoint -> Fill (fPrimaryParticles_fNPoint [itrack][3], fPrimaryParticles_fNMaxPoint [itrack][3]);
            hFitToMaxRatio -> Fill ((Float_t) fPrimaryParticles_fNFitPoint [itrack][3] / fPrimaryParticles_fNMaxPoint [itrack][3]);
            hTrack_chi2 -> Fill (fPrimaryParticles_fPchi2 [itrack]);
            hBx -> Fill (fPrimaryParticles_fBx [itrack]);
            hBy -> Fill (fPrimaryParticles_fBy [itrack]);
...
void CTreeConverterNA49::Finish () {
	outputFile_ -> Write ();
	outputFile_ -> Close ();
}

#endif // CTREECONVERTERNA49_CXX
